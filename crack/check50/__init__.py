...
class Crack(Checks):
    ...
    def cracks_andi(self):
        ...
        self.spawn("./crack 50.jPgLzVirkc").stdout("hi", timeout=20).exit(0)

    @check("compiles")
    def cracks_jason(self):
        """cracks jason's password: 50YHuxoCN9Jkc"""
        self.spawn("./crack 50YHuxoCN9Jkc").stdout("JH", timeout=20).exit(0)

    @check("compiles")
    def cracks_mzlatkova(self):
        """cracks mzlatkova's password: 50CPlMDLT06yY"""
        self.spawn("./crack 50CPlMDLT06yY").stdout("ha", timeout=20).exit(0)

    @check("compiles")
    def cracks_summer(self):
        """cracks summer's password: 50C6B0oz0HWzo"""
        self.spawn("./crack 50C6B0oz0HWzo").stdout("FTW", timeout=20).exit(0)

    @check("compiles")
    def cracks_stelios(self):
        """cracks stelios's password: 50nq4RV/NVU0I"""
        self.spawn("./crack 50nq4RV/NVU0I").stdout("ABC", timeout=20).exit(0)

    @check("compiles")
    def cracks_zamyla(self):
        """cracks zamyla's password: 50i2t3sOSAZtk"""
        self.spawn("./crack 50i2t3sOSAZtk").stdout("lol", timeout=20).exit(0)
