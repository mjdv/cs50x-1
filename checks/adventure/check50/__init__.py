...
class Adventure(Checks):
    ...
    def move_once(self):
        ...
        try:
            self.spawn(RUN_COMMAND + " tiny").stdout(re.escape(room_1_description), str_output=room_1_description)
        except Error as error:
            raise Error(rationale=f"Expected the description of initial "
                                  f"room when Adventure starts.\n    {error}")

        self.spawn(RUN_COMMAND + " tiny").stdin("WEST").stdout(re.escape(room_2_description), str_output=room_2_description)
